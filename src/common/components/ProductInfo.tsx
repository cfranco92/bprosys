...
export const ProductInfo = ({ product }: ProductInfoProps) => {
  const { interestRate, riskLevel, minAmount, term, monthlyFee, annualFee } =
    product;
  ...
  return (
    <div className="bg-white rounded-lg p-6 shadow-sm mb-6">
      ...
      <div className="space-y-1">
        ...
        {minAmount && (
          <InfoRow label="Inversión mínima" value={formatCurrency(minAmount)} />
        )}

        {term && <InfoRow label="Plazo" value={term} />}

        {monthlyFee !== undefined && (
          <InfoRow label="Cuota mensual" value={formatCurrency(monthlyFee)} />
        )}

        {annualFee !== undefined && (
          <InfoRow label="Cuota anual" value={formatCurrency(annualFee)} />
        )}
      </div>
    </div>
  );
